...
- [ ] **Warfare determinism test** (the highest-priority gap — §2) so the living region's war is reproducible. *Half done: battle resolution is now pinned byte-identical (`perf/battle-determin.js`, shipped with the headless kernel). The server warfare tick (musters, campaigns, patrol focus) still has no golden trace — that half stays open.*
...
- [ ] **iOS Safari audit:** AudioContext unlock on first touch (procedural SFX are silent until a gesture), `viewport-fit=cover` + safe-area insets for the notch, prevent double-tap zoom / pull-to-refresh, `touch-action: none` on the canvas. *(Home-screen PWA fullscreen shipped: `manifest.webmanifest` + iOS/Android meta tags launch it chrome-free in landscape; a portrait rotate-gate nudges coarse-pointer devices.)*
- [ ] **Touch parity for the new systems:** the single continuous zoom (now the whole control model — one axis, in to fight / out to command) driven by pinch, F-flares and U-drawer reachable from the touch HUD, battle command (order × pace) usable with thumbs — audit every keyboard-only feature added since the touch layer shipped.
