...
| **his own fist** | the base's fist (finger rest locals) was shaped for the base's thick fingers round its thick hilt, and closes loose on a slighter hand. Profile `fist: { all: {mcp, pip, dip}, index/middle/ring/little: {mcp, pip, dip, spread}, thumb: {cmc, mcp, ip, spread} }` closes each joint by so many degrees about its own hinge (`spread`: about the palm's normal, + toward the thumb; the left side's turns are mirrored). rig.json then says `ownFist`: the game (`MOTION_FINGER` in the clip map) and the editor leave a clip's finger tracks off him — every clip carries the base's fist there, unchanged. The sword's seat is re-read off the new fist by wardrobe.js; profile `grip.slide` (metres) moves the hand down the handle from there. Judge it in the editor with the fingers painted by bone; the gladiator's: `mcp 12 / pip 8`, index `mcp 15`, little `mcp 5, pip -5, spread 30` (open round the hilt), thumb `mcp -10, ip -15` |
