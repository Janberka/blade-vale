...
**2026-09-20 — the kit came off.** The fighter is now the base's OWN man: his skin, the leather wrist bands, Thor's
trousers and boots, and a steel arm either side, all of them meshes of his own in the rig; armour is worn a piece at a
time (`rig.json.wear`, the `gauntL/gauntR/pauldL/pauldR` slots). `retarget.js` — the step that warped the old warrior's
armour, cloak, shield and sword onto him — is no longer in the flow; `dress.js` replaces it, and the pieces it dresses him
in are built and judged in **the char editor, `tools/chared/`** (read its README first: it owns the armour styles). What is
...
| `swiftc -O usdanim.swift` → `node motion.js <anim.json> <id> "<Name>"` | **motions**: a USD skeleton's bound clip (+ the hip's travel, which exporters leave on an Xform above the skeleton) → JSON, then retargeted onto the base's 60 bones in world space → `tools/chared/motions/<id>.json` for the editor's MOTION panel. See `tools/chared/README.md`, "Motions" |
| `python3 ../../chared/serve.py` | **the char editor** (`tools/chared/`): the game's rig, the ITEMS panel, the armour generator and the bake. This is where the character is judged |
| `viewer.html` | *(the old scratch viewer the base was built against — the char editor replaced it)* | the pane viewer (serve the scratch `view/` folder: three r128, `?m=base,warrior`, `?hide=…`, `?cls=1` colours the body by class, `?nn=1` no normal maps; `__view(az, el, dist, cy, cx)`, `__save(name)`) |

What the game does with it (game.js): `rig.json.wear` names every ware mesh (its kind, the market slot that carries it,
the body parts it covers, the other wares it takes off) — `lookRoll` builds `look.wear` from the fighter's gear (a ware's
item id IS its mesh name), `lookApply` shows and hides them, `lookDraw` drops the body under them; `loadModelRig` reads one texture per material and the rig's own head measures
