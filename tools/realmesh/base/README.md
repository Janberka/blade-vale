...
| `node retarget.js` | the warrior kit onto the base, PUSH-OUT ONLY: per body segment (torso to the neck proper, head, arm/fore/hand, thigh/shin/foot ×2) every kit vertex keeps its place along and round the bone and its OWN radius, unless the body reaches past it there — five radial casts against the whole body (its own ray and ±3.5 cm / ±7° round it, the ray's first exit from the flesh) give the flesh's outermost radius, and the vertex sits MARGIN (2 cm; the cloak 5 cm) outside it plus up to 3 cm of its relief over the kit's inner layer, so a collar shrinks to the neck, a pauldron that clears the deltoid stays put, a cuirass hugs the chest; the head is rigid (eyes on the eyes, scaled by the heads' width ratio); the sword and shield move by translation to the new hand / forearm bone and scale ×1.123 (hips 1.092 / 0.972); the Corinthian helm gets the same head carry |
