...
function retargetSkinned(m, o = {}) { const nv = m.pos.length / 3, pos = new Float32Array(nv * 3), ji = new Uint16Array(nv * 4), w = new Float32Array(nv * 4);
  const bySeg = new Map(); for (let v = 0; v < nv; v++) { let best = -1, bw = 0; for (let c = 0; c < 4; c++) if (m.w[v*4+c] > bw) { bw = m.w[v*4+c]; best = m.ji[v*4+c]; } let S = segOfSrcBone.get(sN[m.joints[best]]); if (S && o.viaTorso && o.viaTorso(v) && /^(thigh|shin)/.test(S.name)) S = SEG[0]; if (S) (bySeg.get(S) || bySeg.set(S, []).get(S)).push(v); }
...
for (const m of SRC.meshes) { const short = m.name.replace('FantasyWarrior_', '').replace('_6_characters_0', ''); if (short === 'body') continue; let r;
  ...
  if (short === 'sword') r = carry(m, same(sPos('n41')), same(tPos('handR')), 'handR');
    ...
    { const hj = new Set(SEG.filter(S => S.name.startsWith('hand')).flatMap(S => S.srcBones).map(n => m.joints.indexOf(sN.indexOf(n)))); const hs = { L: SEG.find(S => S.name === 'handL'), R: SEG.find(S => S.name === 'handR') };
      let dropped = 0; const idx = Uint32Array.from(m.idx); for (let t = 0; t < idx.length; t += 3) { let all = true, tsum = 0; for (let k = 0; k < 3; k++) { const v = idx[t + k]; let best = -1, bw = 0; for (let c = 0; c < 4; c++) if (m.w[v*4+c] > bw) { bw = m.w[v*4+c]; best = m.ji[v*4+c]; } if (!hj.has(best)) { all = false; break; } const S = m.pos[v*3] > 0 ? hs.L : hs.R; tsum += local(S.fs, [m.pos[v*3], m.pos[v*3+1], m.pos[v*3+2]]).t; }
        if (all && tsum / 3 > 0.12) { idx[t + 1] = idx[t]; idx[t + 2] = idx[t]; dropped++; } } m.idx = idx; console.log('gauntlet plates dropped:', dropped, 'triangles'); }
