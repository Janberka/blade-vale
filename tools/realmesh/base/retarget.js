...
const WARRIOR = process.env.WARRIOR || path.join(__dirname, '../../../assets/rigs/warrior');
const OUT = process.argv[2] || 'build/base'; const MARGIN = +(process.env.MARGIN || 0.02), SCALE = +(process.env.SCALE || 1.123);   // (margin: plate sits this far off the skin; scale: props grow with the man — Thor's hips 1.092 / the warrior's 0.972)
const SRC = G.read(WARRIOR); const SW = G.bindWorlds(SRC); const sN = SRC.names; const sPos = nm => G.translation(SW[sN.indexOf(nm)]);
...
const warp = (S, p, o = {}) => { const l = local(S.fs, p); if (S.rigid) return S.rigid(p); if (S.name.startsWith('hand')) o = { margin: o.margin, lift: 0 }; if (o.waist && S.name === 'torso') { const d = (l.t - 0.05) / 0.16; l.t += o.waist * Math.exp(-d * d) / S.ft.L; } if (o.lift && l.t > 0.72) { const f = Math.min(1, (l.t - 0.72) / 0.28), sf = f * f * (3 - 2 * f), back = Math.max(0, Math.min(1, 0.5 - 0.5 * Math.sin(l.th) + 0.15)); l.t += o.lift * sf * back / S.ft.L; } /* (lift: model units, the cuirass' back and sides rise over the traps) */ const mg = o.margin != null ? o.margin : MARGIN; const beyond = S.name.startsWith('fore') && l.t > 0.98; const R = (S.name.startsWith('hand') || beyond) ? -1 : hitR(S, l); const own = R > 0 ? Math.max(0, R + mg - l.r) : 0; let push = beyond && o.field ? fieldAt(o.field, l.t, l.th) : o.field ? Math.min(fieldAt(o.field, l.t, l.th), own + 0.04) : own; if (R > 0 && l.r + push < R + mg * 0.5) push = R + mg * 0.5 - l.r; /* (beyond the wrist — a sleeve's cuff — the band's push alone: cast against the fingers, neighbours jumped apart into a sawtooth) */ const rNew = l.r + push; const k = l.r > 1e-4 ? rNew / l.r : 1; return V3.add(V3.add(S.ft.A, V3.scale(S.ft.d, l.t * S.ft.L)), V3.add(V3.scale(S.ft.u, l.a * k), V3.scale(S.ft.v, l.b * k))); };
...
{ const S = SEG.find(x => x.name === 'head'); const sb = SRC.meshes.find(m => /body/.test(m.name)); const spj = JSON.parse(fs.readFileSync(WARRIOR + '/pieces.json')).body; const sock = spj.classes.indexOf('socket'), hd = spj.classes.indexOf('head');
...
function retargetSkinned(m, o = {}) { const nv = m.pos.length / 3, pos = new Float32Array(nv * 3), ji = new Uint16Array(nv * 4), w = new Float32Array(nv * 4);
  const bySeg = new Map(); for (let v = 0; v < nv; v++) { let best = -1, bw = 0; for (let c = 0; c < 4; c++) if (m.w[v*4+c] > bw) { bw = m.w[v*4+c]; best = m.ji[v*4+c]; } let S = segOfSrcBone.get(sN[m.joints[best]]); if (S && o.viaTorso && o.viaTorso(v) && /^(thigh|shin)/.test(S.name)) S = SEG[0]; if (S && S.name.startsWith('hand')) S = SEG.find(x => x.name === 'fore' + S.name.slice(4)); if (S) (bySeg.get(S) || bySeg.set(S, []).get(S)).push(v); }
  ...
  for (let v = 0; v < nv; v++) { const p = [m.pos[v*3], m.pos[v*3+1], m.pos[v*3+2]]; let acc = [0, 0, 0], tot = 0; const wm = new Map();
    for (let c = 0; c < 4; c++) { const wt = m.w[v*4+c]; if (wt <= 0) continue; const sb = sN[m.joints[m.ji[v*4+c]]]; let S = segOfSrcBone.get(sb); if (!S) { console.log('unmapped bone', sb); continue; }
      if (S.name.startsWith('hand')) S = SEG.find(x => x.name === 'fore' + S.name.slice(4));   // (the cuff round the wrist: the forearm's warp AND the forearm's bone — on the hand's it tore from the vambrace as spikes whenever the wrist turned)
      const tb0 = tN.indexOf(S.boneMap[sb] || S.boneMap[S.srcBones[0]]); if (o.viaTorso && o.viaTorso(v) && /^(thigh|shin)/.test(S.name)) S = SEG[0];   // (the skirt: its place follows the torso, its weight the leg)
      const q = warp(S, p, { margin: o.margin, field: fields.get(S), lift: o.liftOf ? o.liftOf(v, S) : 0, waist: o.waist || 0 }); acc = V3.add(acc, V3.scale(q, wt)); tot += wt; wm.set(tb0, (wm.get(tb0) || 0) + wt); }
...
for (const m of SRC.meshes) { const short = m.name.replace('FantasyWarrior_', '').replace('_6_characters_0', ''); if (short === 'body') continue; let r;
  ...
  else if (short === 'armor') { const pj = JSON.parse(fs.readFileSync(WARRIOR + '/pieces.json')).armor, cu = pj.classes.indexOf('cuirass');
    // THE GAUNTLET PLATES over the back of the hand (kit triangles whose three vertices ride a hand bone, out past the wrist) come off: the base has
    // hands of its own, and on them the plates read as a second set of fingers. Collapsed to nothing, not removed — pieces.json counts triangles.
    { const hj = new Set(SEG.filter(S => S.name.startsWith('hand')).flatMap(S => S.srcBones).map(n => m.joints.indexOf(sN.indexOf(n)))); const hs = { L: SEG.find(S => S.name === 'handL'), R: SEG.find(S => S.name === 'handR') };
      let dropped = 0; const idx = Uint32Array.from(m.idx); const plateV = new Uint8Array(m.pos.length / 3); for (let v = 0; v < plateV.length; v++) { let best = -1, bw = 0; for (let c = 0; c < 4; c++) if (m.w[v*4+c] > bw) { bw = m.w[v*4+c]; best = m.ji[v*4+c]; } if (!hj.has(best)) continue; const S = m.pos[v*3] > 0 ? hs.L : hs.R; if (local(S.fs, [m.pos[v*3], m.pos[v*3+1], m.pos[v*3+2]]).t > 0.3) plateV[v] = 1; }   /* (past 30 % of a hand's length: the plates over the back of the hand, not the cuff's own lower row — dropped at 12 % the cuff's rim became a sawtooth) */
      for (let t = 0; t < idx.length; t += 3) if (plateV[idx[t]] || plateV[idx[t+1]] || plateV[idx[t+2]]) { idx[t + 1] = idx[t]; idx[t + 2] = idx[t]; dropped++; } /* (any triangle touching a plate vertex — the cuff's fringe reaching them was a ring of spikes) */ m.idx = idx; console.log('gauntlet plates dropped:', dropped, 'triangles'); }
    const SK = new Set(['skirt', 'skirtTop', 'straps', 'pouch'].map(c => pj.classes.indexOf(c))); r = retargetSkinned(m, { liftOf: (v, S) => S.name === 'torso' && pj.vclass[v] !== pj.classes.indexOf('pauldron') ? 0.09 : 0, viaTorso: v => SK.has(pj.vclass[v]), waist: 0.08 }); }   // (the collar: 9 cm higher at the back over the traps — every piece there rises together, or the layers tear apart; the pauldrons ride the arms)
...
for (const m of out) { const nv = m.pos.length / 3, mm = M.empty(); mm.pos = Array.from(m.pos); const vid = M.weldIds(mm), acc = new Float64Array(nv * 3);   // smooth normals welded by position, every vertex kept (a dropped plate leaves its vertices unused — pieces.json counts them)
  for (let t = 0; t < m.idx.length; t += 3) { const a = m.idx[t], b = m.idx[t+1], c = m.idx[t+2]; if (a === b || b === c) continue; const A = M.vpos(mm, a), Bp = M.vpos(mm, b), C = M.vpos(mm, c); const n = V3.cross(V3.sub(Bp, A), V3.sub(C, A)); for (const i of [a, b, c]) { const w = vid[i]; acc[w*3] += n[0]; acc[w*3+1] += n[1]; acc[w*3+2] += n[2]; } }
  const nrm = new Float32Array(nv * 3); for (let v = 0; v < nv; v++) { const w = vid[v]; const n = V3.norm([acc[w*3], acc[w*3+1], acc[w*3+2]]); nrm[v*3] = n[0]; nrm[v*3+1] = n[1]; nrm[v*3+2] = n[2]; } m.nrm = nrm; }
...
const extras = T.meshes.filter(m => m.name !== 'base_body').map(m => ({ name: m.name, pos: m.pos, nrm: m.nrm, uv: m.uv, ji: m.ji, w: m.w, idx: m.idx, material: m.name === 'hair_long' ? 2 : 3 }));   // (the hair cards on the hair material; everything else the base brought — Thor's wraps, trousers, boots, steel arms — through as it is, on the two-sided wear material)
...
fs.copyFileSync(WARRIOR + '/6_characters_baseColor.jpg', OUT + '/6_characters_baseColor.jpg');
G.write(OUT, { meshes, skeleton: skel, materials: [{ name: 'base', pbrMetallicRoughness: { baseColorTexture: { index: 0 }, metallicFactor: 0, roughnessFactor: 0.9 } }, { name: 'kit', pbrMetallicRoughness: { baseColorTexture: { index: 1 }, metallicFactor: 0, roughnessFactor: 0.9 } }, { name: 'hair', pbrMetallicRoughness: { baseColorTexture: { index: 2 }, metallicFactor: 0, roughnessFactor: 0.9 }, alphaMode: 'MASK', alphaCutoff: 0.5, doubleSided: true }, { name: 'wear', pbrMetallicRoughness: { baseColorTexture: { index: 0 }, metallicFactor: 0, roughnessFactor: 0.85 }, doubleSided: true }], images: ['atlas.jpg', '6_characters_baseColor.jpg', 'hair.png'] });   // (no normal map: the game's surface pass draws the skin's detail; the viewer's normals were the Thor maps)
fs.copyFileSync(WARRIOR + '/pieces.json', OUT + '/pieces.json');
...
{ const S = SEG.find(x => x.name === 'head'); const hb = WARRIOR + '/helm_corinthian'; const h = JSON.parse(fs.readFileSync(hb + '.json')); const buf = fs.readFileSync(hb + '.bin'); const ab = buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength);
